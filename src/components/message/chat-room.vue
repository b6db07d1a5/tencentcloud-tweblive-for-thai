    ...
    <div id="message-send-box-wrapper" v-show="this.tabSelected ===0">
      ...
      <div class="send-header-bar">
        <el-popover placement="top" width="400" trigger="click">
          ...
          <i class="iconfont icon-smile" slot="reference" title="Make emoticons"></i>
        </el-popover>
        <div class="login-btn" v-if="!isSDKReady">
          Visitor Status Please
          <a @click="checkLogin" style="color:#23ade5;">Login</a>
        </div>
        <el-input
          v-else
          placeholder="Text~"
        ...
        <div class="send-btn">
          <el-button type="primary" @click="sendTextMessage" round>Send</el-button>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    // If you roll to the bottom, keep it at the bottom, otherwise you will be prompted whether to roll to the bottom
    keepMessageListOnButtom() {
      let messageListNode = this.$refs['message-list'];
      if (!messageListNode) {
        return;
      }
      // Forced to scroll to the bottom within 20px from the bottom, otherwise it will prompt a new message
      ...
      if (!this.isSDKReady) {
        ...
        this.tweblive
          ...
          .catch(error => {
            message.status = 'fail';
            if (error.code === 80001) {
              error.message = 'The text may contain sensitive words';
            }
            this.$store.commit('showMessage', {
              type: 'error',
