<template>
  <div class="live-like">
    <div
      ...
      :key="index"
      style="opacity: 1; z-index: 0; transform: translate3d(0px, 0px, 0px);"
    >
  ...
  methods: {
    ...
    likeClick() {
      ...
      const anmationData1 = {
        ...
        // imageIndex:Math.floor(Math.random() * (4 - 0 + 1)) + 0,
        factor: {
          speed: 0.005, // Movement speed, the smaller the value, the slower
          t: 0 //  Bessel function coefficient
        }
      };
    ...
    bubbleAnimate() {
      this.queue.forEach(item => {
        ...
        if (anmationData.factor.t > 1) {
          this.queue.splice(
            this.queue.findIndex(item => item.id === anmationData.id),
            1
          );
        }
      });
